...
function getColorStreaks() {
    ...
    for (let i = 0; i < RESULTS.length; i++) {
        let c = getColor(RESULTS[i]);
        if (c === 'green') continue; // Ignore green for pattern matching
        ...
        if (curColor === null) {
            ...
            streaks.push({ color: curColor, count: count });
            curColor = c;
            count = 1;
        }
    }
...
function checkColorSignal() {
    if (RESULTS.length < 4) return null;
    let r = RESULTS.slice().reverse().map(n => getColor(n) === 'green' ? 'G' : (getColor(n) === 'red' ? 'R' : 'B'));
    
    // Pola Sandera: Hitam, Merah, Merah, Hitam -> Target Merah 
    if (r.length >= 4) {
        if (r[0] === 'B' && r[1] === 'R' && r[2] === 'R' && r[3] === 'B') {
            return { target: 'red', pattern: 'Pola Sandera (Hitam-Merah-Merah-Hitam)' };
        }
    }
    
    // [TRUE ML] Pola Pancingan Hitam: Merah, Merah, Merah, Merah, Hitam -> Target Merah 
    if (r.length >= 5) {
        if (r[0] === 'B' && r[1] === 'R' && r[2] === 'R' && r[3] === 'R' && r[4] === 'R') {
            return { target: 'red', pattern: 'Pola Pancingan Hitam (M-M-M-M-H)' };
        }
    }

    // [NEW ML] Pola Sandera Merah: Merah, Merah, Hitam, Merah, Merah -> Target Merah (76.4%)
    if (r.length >= 5) {
        if (r[0] === 'R' && r[1] === 'R' && r[2] === 'B' && r[3] === 'R' && r[4] === 'R') {
            return { target: 'red', pattern: 'Pola Sandera Merah (M-M-H-M-M)' };
        }
    }

    // [NEW ML] Pola 2 Merah 2 Hitam: Merah, Merah, Hitam, Hitam -> Target Merah (66.6%, High Vol)
    if (r.length >= 4) {
        if (r[0] === 'B' && r[1] === 'B' && r[2] === 'R' && r[3] === 'R') {
            return { target: 'red', pattern: 'Pola 2 Merah 2 Hitam (M-M-H-H)' };
        }
    }
...
function getOddEvenStreaks() {
    ...
    for (let i = 0; i < RESULTS.length; i++) {
        let num = RESULTS[i];
        if (num === 0) continue; // Ignore green for pattern matching
        let t = (num % 2 !== 0) ? 'odd' : 'even';
        ...
        if (curType === null) {
            ...
            streaks.push({ type: curType, count: count });
            curType = t;
            count = 1;
        }
    }
...
function checkOddEvenSignal() {
    if (RESULTS.length < 4) return null;
    let r = RESULTS.slice().reverse().map(n => n === 0 ? 'G' : (n % 2 !== 0 ? 'O' : 'E'));
    
    // Pola 3 Genap 1 Ganjil -> Target Genap
    if (r.length >= 4) {
        if (r[0] === 'O' && r[1] === 'E' && r[2] === 'E' && r[3] === 'E') {
            return { target: 'even', pattern: 'Pola Genap Dominan (Genap-Genap-Genap-Ganjil)' };
        }
    }
    
    // Pola Zig-Zag Pincang -> Target Genap
    if (r.length >= 5) {
        if (r[0] === 'O' && r[1] === 'E' && r[2] === 'O' && r[3] === 'E' && r[4] === 'E') {
            return { target: 'even', pattern: 'Pola Zig-Zag Pincang (Genap-Genap-Ganjil-Genap-Ganjil)' };
        }
    }
    
    // Pola Zig-Zag Murni -> Target Genap
    if (r.length >= 4) {
        if (r[0] === 'O' && r[1] === 'E' && r[2] === 'O' && r[3] === 'E') {
            return { target: 'even', pattern: 'Pola Zig-Zag Murni (Genap-Ganjil-Genap-Ganjil)' };
        }
    }
    
    // [TRUE ML] Pola Ganjil Terjepit: Genap, Ganjil, Genap, Genap, Ganjil -> Target Ganjil 
    if (r.length >= 5) {
        if (r[0] === 'O' && r[1] === 'E' && r[2] === 'E' && r[3] === 'O' && r[4] === 'E') {
            return { target: 'odd', pattern: 'Pola Ganjil Terjepit (Ge-Ga-Ge-Ge-Ga)' };
        }
    }

    // [TRUE ML] Pola Zig-Zag Sempurna: Ganjil, Genap, Ganjil, Genap, Ganjil -> Target Genap 
    if (r.length >= 5) {
        if (r[0] === 'O' && r[1] === 'E' && r[2] === 'O' && r[3] === 'E' && r[4] === 'O') {
            return { target: 'even', pattern: 'Pola Zig-Zag Sempurna (Ga-Ge-Ga-Ge-Ga)' };
        }
    }

    // [TRUE ML] Pola Pemutus Ganjil 5: Ganjil, Ganjil, Ganjil, Ganjil, Ganjil -> Target Genap 
    if (r.length >= 5) {
        if (r[0] === 'O' && r[1] === 'O' && r[2] === 'O' && r[3] === 'O' && r[4] === 'O') {
            return { target: 'even', pattern: 'Pola Pemutus Ganjil 5 (Ga-Ga-Ga-Ga-Ga)' };
        }
    }

    // [NEW ML] Pola Terjepit Ganjil: Ganjil, Genap, Ganjil, Ganjil, Genap -> Target Genap (70.5%)
    if (r.length >= 5) {
        if (r[0] === 'E' && r[1] === 'O' && r[2] === 'O' && r[3] === 'E' && r[4] === 'O') {
            return { target: 'even', pattern: 'Pola Terjepit Ganjil (Ga-Ge-Ga-Ga-Ge)' };
        }
    }

    // [NEW ML] Pola Triple Genap: Genap, Ganjil, Genap, Genap, Genap -> Target Ganjil (69.2%, High Vol)
    if (r.length >= 5) {
        if (r[0] === 'E' && r[1] === 'E' && r[2] === 'E' && r[3] === 'O' && r[4] === 'E') {
            return { target: 'odd', pattern: 'Pola Triple Genap (Ge-Ga-Ge-Ge-Ge)' };
        }
    }
...
function checkHighLowSignal() {
    if (RESULTS.length < 4) return null;
    let r = RESULTS.slice().reverse().map(n => n === 0 ? 'G' : (n >= 19 ? 'H' : 'L'));
    
    // Pola Ping-Pong Ekstrim -> Target Kecil
    if (r.length >= 5) {
        if (r[0] === 'H' && r[1] === 'L' && r[2] === 'H' && r[3] === 'L' && r[4] === 'H') {
            return { target: 'low', pattern: 'Pola Pingpong Ekstrim (B-K-B-K-B)' };
        }
    }
    
    // Pola Capit -> Target Kecil
    if (r.length >= 5) {
        if (r[0] === 'H' && r[1] === 'H' && r[2] === 'L' && r[3] === 'H' && r[4] === 'H') {
            return { target: 'low', pattern: 'Pola Capit (B-B-K-B-B)' };
        }
    }
    
    // [TRUE ML] Pola Jebakan Besar: Besar, Besar, Besar, Kecil, Besar -> Target Kecil 
    if (r.length >= 5) {
        if (r[0] === 'H' && r[1] === 'L' && r[2] === 'H' && r[3] === 'H' && r[4] === 'H') {
            return { target: 'low', pattern: 'Pola Jebakan Besar (B-B-B-K-B)' };
        }
    }

    // [TRUE ML] Pola Cermin Besar: Besar, Besar, Kecil, Kecil, Besar -> Target Kecil 
    if (r.length >= 5) {
        if (r[0] === 'H' && r[1] === 'L' && r[2] === 'L' && r[3] === 'H' && r[4] === 'H') {
            return { target: 'low', pattern: 'Pola Cermin Besar (B-B-K-K-B)' };
        }
    }

    // [NEW ML] Pola Tiga Besar: Besar, Kecil, Besar, Besar, Besar -> Target Kecil (70.5%)
    if (r.length >= 5) {
        if (r[0] === 'H' && r[1] === 'H' && r[2] === 'H' && r[3] === 'L' && r[4] === 'H') {
            return { target: 'low', pattern: 'Pola Tiga Besar (B-K-B-B-B)' };
        }
    }
...
function capturePredictions() {
    ...
    const colorSig = typeof checkColorSignal === 'function' ? checkColorSignal() : null;
    const oeSig = typeof checkOddEvenSignal === 'function' ? checkOddEvenSignal() : null;
    const hlSig = typeof checkHighLowSignal === 'function' ? checkHighLowSignal() : null;
...
function renderPredictionHero() {
    ...
    if (predEkorHitamEl && predEkorMerahEl) {
        ...
        const colorSignal = checkColorSignal();
    ...
    if (predEkorGanjilEl && predEkorGenapEl) {
        ...
        const oeSignal = checkOddEvenSignal();
    ...
    if (predEkorBesarEl && predEkorKecilEl) {
        ...
        const hlSignal = checkHighLowSignal();
    ...
    if (liveSignalsEl) {
        ...
        const renderBadge = (sig) => {
            ...
            let text = sig.target.toUpperCase();
        ...
        const currentWarnaSig = typeof checkColorSignal === 'function' ? checkColorSignal() : null;
        const currentOeSig = typeof checkOddEvenSignal === 'function' ? checkOddEvenSignal() : null;
        const currentHlSig = typeof checkHighLowSignal === 'function' ? checkHighLowSignal() : null;
    ...
    if (murniWarnaEl) {
        if (typeof checkColorSignal === 'function') {
            const sig = checkColorSignal();
            if (sig) {
    ...
    if (murniOeEl) {
        if (typeof checkOddEvenSignal === 'function') {
            const sig = checkOddEvenSignal();
            if (sig) {
    ...
    if (murniHlEl) {
        if (typeof checkHighLowSignal === 'function') {
            const sig = checkHighLowSignal();
            if (sig) {
